...
- `trading` — morning day-trading against the Korea Investment **paper** account. Every limit lives in `lib/teams/trading/config.ts` as code, never as prompt; exits are generated by the risk gate and a strategy cannot propose them. `kis.ts` refuses to run against the live host at all. The candidate screen runs inside each tick, not at open: it reads today's accumulated turnover, which is zero before the bell. It costs six ranking calls and nothing else — the response carries shares outstanding and KIS's own traded-value-to-market-cap figure, so no per-ticker quote is needed. Those six are one per market per price band, because the endpoint returns thirty rows ranked by *absolute* turnover, which contradicts a screen looking for turnover relative to size; banding by price took the pool from 60 names to 180.
